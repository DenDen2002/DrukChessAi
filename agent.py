...
class Agent:
    def __init__(self, local_predictions: bool = False, model_path=None, state=chess.STARTING_FEN):
        ...
        if local_predictions and model_path is not None:
            ...
            try:
                ...
                self.socket_to_server.setsockopt(
                ...
                print(
                    f"Agent could not connect to the server at {server}:{port}: ", e)
                exit(1)
    ...
    def predict_server(self, data: np.ndarray):
        ...
        """
        # send data to server
        self.socket_to_server.send(
            f"{len(data.flatten()):010d}".encode('ascii'))

...
model = load_model(config.MODEL_FOLDER +
                   "/model.h5")
...
class ClientHandler(threading.Thread):
    ...
    def receive(self):
        ...
        try:
            ...
            logging.warning(
                f"Connection reset by peer. Client IP: {str(self.address[0])}:{str(self.address[1])}")
...
if __name__ == "__main__":
    ...
    s = ServerSocket(os.environ.get("SOCKET_HOST", "0.0.0.0"),
                     int(os.environ.get("SOCKET_PORT", 5000)))
    s.start()

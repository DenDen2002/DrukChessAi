...
class Game:
    ...
    def play_one_game(self, stochastic: bool = True) -> int:
        ...
        while not self.env.board.is_game_over():
            ...
            logging.info(f"\n{self.env.board}")
            logging.info(
                f"Value according to white: {self.white.mcts.root.value}")
            logging.info(
                f"Value according to black: {self.black.mcts.root.value}")
        ...
        if previous_moves[0] is None or previous_moves[1] is None:
            ...
            try:
                node = current_player.mcts.root.get_edge(
                    previous_moves[0].action).output_node
    ...
    def train_puzzles(self, puzzles: pd.DataFrame):
        ...
        for puzzle in puzzles.itertuples():
            ...
            logging.info(
                f"Puzzle to solve ({puzzle.rating} ELO): {self.env.fen}")
            logging.info(f"\n{self.env.board}")
            ...
            while not self.env.board.is_game_over():
                ...
                if counter > config.MAX_PUZZLE_MOVES:
                    logging.warning(
                        "Puzzle could not be solved within the move limit")
                    break

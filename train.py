...
class Trainer:
    ...
    def train_all_data(self, data):
        ...
        for part in tqdm(range(len(X)//self.batch_size)):
            ...
            losses = self.train_batch(
                X[start:end], y[0][start:end], y[1][start:end])
    ...
    def train_random_batches(self, data):
        ...
        for _ in tqdm(range(2*max(5, len(data) // self.batch_size))):
            indexes = np.random.choice(
                len(data), size=self.batch_size, replace=True)
    ...
    def plot_loss(self, history):
        ...
        df[['loss', 'policy_head_loss', 'value_head_loss']] = df[[
            'loss', 'policy_head_loss', 'value_head_loss']].apply(pd.to_numeric, errors='coerce')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data-folder', type=str,
                        help='The data folder to train on')

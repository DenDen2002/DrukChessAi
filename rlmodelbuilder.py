...
class RLModelBuilder:
    ...
    def build_policy_head(self) -> Model:
        ...
        model.add(Flatten())
        model.add(
            Dense(self.output_shape[0], activation="sigmoid", name='policy_head'))
        return model
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Create the neural network for chess')
    parser.add_argument('--model-folder', type=str,
                        default='models/', help='Folder to save the model')
    ...
    # save the model
    print(
        f"Saving model to {args['model_folder']} as {args['model_name']}.h5 ...")
